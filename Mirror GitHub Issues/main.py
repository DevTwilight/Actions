...
def close_reasons_from_labels(labels):
    label_map = {
        "not planned": "Closed as 'not planned' because the original issue was labeled as such.",
        "duplicate": "Closed as duplicate of the original issue.",
        "completed": "Closed as completed based on the original issue."
    }
    for label in labels:
...
def add_comment(repo, issue_id, comment_body):
    gh_request("POST", f"{GITHUB_API}/repos/{repo}/issues/{issue_id}/comments", HEADERS, json={
        "body": comment_body
    })
...
def mirror_issue(repo):
    ...
    if not mirror_id and issue_state == "open":
        ...
        gh_request("PATCH", f"{GITHUB_API}/repos/{repo}/issues/{mirror_id}", HEADERS, json={
            "title": issue_title,
            "body": body,
            "state": issue_state  # This allows reopen/close sync from source
        })

        if issue_state == "closed":
            reason = close_reasons_from_labels(issue_labels)
            if reason:
                add_comment(repo, mirror_id, reason)
            else:
                add_comment(repo, mirror_id, DEFAULT_CLOSE_COMMENT)
...
def main():
    ...
    if event_repo == SOURCE_REPO:
        ...
        if source_id and issue_state == "closed":
            reason = close_reasons_from_labels(issue_labels)
            if reason:
                add_comment(SOURCE_REPO, source_id, reason)
